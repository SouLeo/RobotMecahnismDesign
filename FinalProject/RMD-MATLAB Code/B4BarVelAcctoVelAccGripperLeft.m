...
%Parametric 2D plot of Gripper XY from top view
figure(3)
%t=Griptheta2;
t=theta2global;
X=GrippointB_Xglobal; % positive or negative for left and right gripper
Y=GrippointB_Yglobal;
plot(X,Y)
xlabel('GripperX (inches)')
ylabel('GripperY (inches)')

%Parametric 2D plot of Gripper XY (YZ) from side view
theta2global=theta2global*pi/180;
figure(4)
%t=theta2global;
t=Griptheta2;
Xside=8.75*cos(theta2global)+2.36738+GrippointB_Yglobal; %(Y)
Yside=8.75*sin(theta2global)+.8375; %(Z)
plot(Xside,Yside)
xlabel('GripperY (inches)')
ylabel('GripperZ (inches)')

%3D plot of gripper motion w.r.t. motor position
figure(32)
plot3(-X,Xside,Yside) % positive or negative for left and right gripper
grid on
xlim([-4 4])
ylim([7 15])
zlim([-7 1])
xlabel('Gripper X (inches)')
ylabel('Gripper Y (inches)')
zlabel('Gripper Z (inches)')
theta2global=theta2global*180/pi;
...
% Acceleration plots
 ...
 %START NEW STUFF relating original B4Bar vel and acc stuff to Gripper vel
 %and acc stuff
 Gripomega2=omega4;
 Gripalpha2=alpha4;
 ...
 figure(26)
 plot(theta2global,Gripomega2)
 xlabel('theta2global (deg)')
 ylabel('Gripomega2 (rad/s)')
 figure(27)
 plot(theta2global,Gripomega3)
 xlabel('theta2global (deg)')
 ylabel('Gripomega3 (rad/s)')
 figure(28)
 plot(theta2global,Gripomega4)
 xlabel('theta2global (deg)')
 ylabel('Gripomega4 (rad/s)')
 
 figure(29)
 plot(theta2global,Gripalpha2)
 xlabel('theta2global (deg)')
 ylabel('Gripalpha2 (rad/s^2)')
 figure(30)
 plot(theta2global,Gripalpha3)
 xlabel('theta2global (deg)')
 ylabel('Gripalpha3 (rad/s^2)')
 figure(31)
 plot(theta2global,Gripalpha4)
 xlabel('theta2global (deg)')
 ylabel('Gripalpha4 (rad/s^2)')
